...
| 2026-09-21 | **A fence going away from you is a chain of overlapping boards, not a texture.** Each picket stands as tall as the ones facing you, so the one in front hides all but the top of the one behind: what you see is the same eight-pixel pitch as the boards across, each with its lit top edge and the dark line where the next tucks under it. The edges of the run are unbroken down its whole length and the joins only cut across the middle — carried right across, they chopped it into blocks and it read as a ladder. Four earlier goes failed the same way: two thin rails was a ladder, one fat rail a plank, spaced posts on a rail neither, and a smooth bar with the tone wandering along it a stack of crates. |
| 2026-09-21 | **A picket is four wide with four of daylight beside it, a pale face, and a dark cut head on top.** The head is the only part of a picket that is not the colour of its neighbour, so it is what tells one from the next at a glance; the face is lighter than the rails behind it, without which a run of them is a fence-coloured hedge. And a post is that same board a pixel wider and two taller, not the stout thing it was — a bollard every time the fence turned a corner. |
