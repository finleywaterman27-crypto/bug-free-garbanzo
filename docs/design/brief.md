...
| 2026-09-21 | **A fence going away from you is a line of POSTS overlapping one another, drawn face on — settled from a reference, not from first principles.** Stardew Valley's modding wiki: "In a vertical line of fences, the top 16 pixels of the bottom neighbor piece overlap the bottom 16 pixels of the top neighbor piece." So the games that have solved this do not draw the fence from a second angle at all — every piece is face on and the run reads because each post leans over the one behind it. Ours is a post every eight pixels, measured from the foot of the fence so the pitch carries across the tile joins — the same pitch as the boards across, so a fence going away is spaced exactly like a fence coming towards you, checked by counting the pixels between marks rather than by eye — each the same board as every picket — same width, same height — cut level with the top of the fence at the far end. Everything that tried to draw the true side view failed: two thin rails was a ladder, one fat rail a plank, the whole panel turned on its side a trellis, and a bar with the tone wandering along it a stack of crates. |
...
| 2026-09-21 | **Only the fingertips of a hand are rounded off.** Rounding the top of it too punched a one-pixel hole either side of the wrist, where the hand meets the arm it is part of, and the outline pass filled each hole with a dark pixel: two dots on every hand, which at this size read as something the character was holding. They had always been there; the shorter body made them big enough to see. |
| 2026-09-21 | **Twelve frames, not four, and two clocks rather than one.** Four was a flick-book. The sea now runs a whole swell in about two and a half seconds and a tree takes six seconds over one lean and back, because the same pace for both made the sea look like a flag and the trees like a fan. Three times the frames costs three times the cached pictures of anything that moves — and only things that move are cached per frame. |
| 2026-09-21 | **The sea runs one way for ever, and is not baked into the map at all.** The light on the water is a strip as wide as its own repeat, drawn over the sea and slid sideways a pixel at a time at nine pixels a second: it leaves the right-hand edge exactly as it enters the left. Baked into the map, a sea that drifts one way has to snap back at the end of its cycle — a hash of a drifting coordinate never loops — which is why it swelled back and forth instead, and why that was wrong. Drawing it rather than baking it also unties the speed from the frame rate: a pixel at a time is as smooth as a pixel will allow. |
| 2026-09-21 | **The island is painted once, and only the waterline is repainted per frame.** Forty pixels of it, cropped to the rows the tide lives in. Painting the whole map a dozen times over to animate that band was three of the four seconds the page spent starting up; it is now about one. |
| 2026-09-21 | **A tree is mostly still.** Twenty-four frames of sway rather than twelve, and at rest for three quarters of them: it leans over once in a while and comes back, over three quarters of a minute. One movement somewhere on the island every four seconds or so. Twelve frames of constant wagging was a fan, four was a metronome. |
| 2026-09-21 | **The clocks are spread evenly around the cycle, and worked backwards from WHEN EACH TREE SHOULD LEAN.** Offsets drawn at random from where a thing stands landed in clumps — three leaning within half a second of one another and then nothing for four — and a clump is precisely what the eye reads as everything moving together. Spacing the clocks evenly is not enough either, because a palm leans at a different point of its cycle from a broadleaf, so two evenly spaced clocks still put two of them over on the same second. The page now asks the prop when it leans and sets the clock to land that moment where it wants it: one movement somewhere on the island about every one and a half seconds, and never two together. |
| 2026-09-21 | **The hour of the day is one colour, multiplied over the finished picture.** Not a second island painted for every hour, and not a second set of every prop: the island is painted in daylight and the whole frame is multiplied by the colour of the light. That is what sunlight does — light of a colour falls on a surface of a colour and you get the two multiplied — so a white noon changes nothing, an amber evening warms everything by as much as its own colour allows, and a blue night darkens the lot without flattening it. |
| 2026-09-21 | **The night stops at four tenths of daylight, and keeps its hue.** A game you play in the evening is not improved by a screen you have to squint at. The suite checks every minute of the day: the light is a colour, it never falls below that floor, it never moves more than four steps in a minute, and it meets itself at midnight. |
| 2026-09-21 | **A lit window goes on AFTER the evening, and anyone standing in front of one is drawn again over it.** A lamp is a light, not a surface: dimmed along with everything else it would be a slightly less dark window. Which means whoever is standing in the light would be painted over by it, so they are redrawn on top and the evening laid over just them — lit by the room rather than cut out of it. |
| 2026-09-21 | **The island keeps your clock; the test rig keeps a slider.** The game reads the time off the device, the way this kind of game does. The page for looking at the island has an hour you can drag, because that is what it is for. |
| 2026-09-21 | **Ten walk frames a second rather than seven and a half.** The number is a cadence, not a speed: the frame turns over every `STRIDE` pixels travelled, so the feet keep hold of the ground however fast the character goes, and all this sets is how briskly the legs go round. Forty-five pixels a second was a stroll; sixty is a walk — about two tiles a second. |
| 2026-09-21 | **A canopy bends; it does not slide.** How high a cluster of leaves sits decides when it goes: the crown leads and the lower branches follow a beat behind, so the lean travels down through the tree and back up again. Two steps a cycle become six, each one smaller, for the same one pixel of travel — which is the whole of what smoother means when a pixel is the smallest thing you can move. The palm's fronds whip three frames behind its crown, for the same reason. |
| 2026-09-21 | **Reversed a claim: the tree never moved at all.** It was declared as something that sways — and so cost twenty-four cached pictures of itself — and then drawn identically on every frame. The only things moving on the island were the palms and the bushes. Worse, both of those passed an amount of their own to the sway rather than the one they declare, so halving what they declared did nothing whatever to what they drew: the palms had been swinging their old two pixels the whole time. |
| 2026-09-21 | **Every tree is on its own clock, not merely its own frame.** Sharing one clock, the whole field turned over on the same tick: the same movement at the same instant, which is a metronome rather than weather, and you see it the moment you stop watching any one tree. Each prop's clock is offset by a fraction of a second drawn from where it stands, so it is the same tree on the same beat every time the page opens and no two of them change together. |
| 2026-09-21 | **Half the sway on the broadleaves and the bushes, half on the palms.** A tree leans one pixel one way now rather than one either way; a palm one either way rather than two. And everything that moves moves through a whole sine over the cycle, so it comes back to where it started — a loop with a jump in it is worse than no loop. |
| 2026-09-21 | **The sea swells sideways ten pixels each way** rather than sitting still with a few streaks drifting. It is the same water sliding back and forth, which is what the eye reads as flowing, and because it returns to where it began the loop is seamless. Light streaks ride the swell rather than marching one way and snapping back: a hash of a drifting coordinate never loops. |
